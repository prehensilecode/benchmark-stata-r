...
out <- append(out, time(DT <- fread(file.path(datadir, "1e7.csv"), data.table = FALSE)))

names <- append(names, "save binary")
out <- append(out, time(write.fst(DT, file.path(datadir, "1e7.fst"))))

names <- append(names, "open binary")
out <- append(out, time(DT <- read.fst(file.path(datadir, "1e7.fst"))))
...
# merge 
DT <- read.fst(file.path(datadir, "1e7.fst"))
setDT(DT)
f <- function(){
	DT_merge <- read.fst(file.path(datadir, "merge_string.fst"))
	setDT(DT_merge)
	setkey(DT, id1, id3)
...
setDT(DT)
f <- function(){
	DT_merge <- read.fst(file.path(datadir, "merge_int.fst"))
	setDT(DT_merge)
	setkey(DT, id4, id6)
...
out <- append(out, time(ggsave(file.path(datadir, "plot.pdf"), ggplot(DT1[1:1000], aes(x = v1, y = v2)) +  geom_point())))

# run benchmark
fwrite(data.table(command = names, result = out), file.path(datadir, "resultR1e7.csv"))
